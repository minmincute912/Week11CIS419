...
class PolynomialRegression:
    ...
    def fit(self, X, y):
        ...
        Xpoly = np.c_[np.ones(Xpoly.shape[0]),Xpoly]
        regTerm = self.regLambda * np.eye(Xpoly.shape[1])
        self.theta = np.linalg.inv(Xpoly.T.dot(Xpoly)+ regTerm).dot(Xpoly.T).dot(y)
    ...
    def predict(self, X):
        ...
        prediction = Xpoly.dot(self.theta)
        return prediction
        # TODO
